...
public class RegistroUsoController {
    ...
    @GetMapping("/paged")
    public ResponseEntity<com.example.safestock.adapter.inbound.dto.PagedResponse<RegistroUsoResponse>> listarPaginado(
        ...
        // Converte RegistroUso para RegistroUsoResponse com nome do funcionário
        List<RegistroUsoResponse> responseList = result.getContent().stream()
                .map(registroUso -> {
                    RegistroUsoResponse resp = new RegistroUsoResponse();
                    resp.setId(registroUso.getId());
                    resp.setProduto(registroUso.getProduto());
                    resp.setDataValidade(registroUso.getDataValidade());
                    resp.setQuantidade(registroUso.getQuantidade());
                    resp.setDataHoraSaida(registroUso.getDataHoraSaida());
                    
                    if (registroUso.getFuncionario() != null) {
                        resp.setFuncionarioId(registroUso.getFuncionario().getId());
                        String nomeCompleto = registroUso.getFuncionario().getNome() + 
                                             (registroUso.getFuncionario().getSobrenome() != null 
                                              ? " " + registroUso.getFuncionario().getSobrenome() 
                                              : "");
                        resp.setResponsavel(nomeCompleto.trim());
                    }
        ...
        // Cria um novo PagedResult com os DTOs
        com.example.safestock.domain.model.PagedResult<RegistroUsoResponse> responseResult = 
            new com.example.safestock.domain.model.PagedResult<>(
                responseList,
                result.getPage(),
                result.getSize(),
        ...
        return ResponseEntity.ok(com.example.safestock.adapter.inbound.dto.PagedResponse.from(responseResult));
    }
